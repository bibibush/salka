...
describe('AnalysisResultCard', () => {
  it('전체 점수와 요약을 표시한다', () => {
    ...
    expect(screen.getByText('전반적으로 무난한 편입니다')).toBeInTheDocument();
  });

  it('개별 성분 평가(이름과 사유)를 표시한다', () => {
    render(<AnalysisResultCard result={result} />);
    expect(screen.getByText('Niacinamide')).toBeInTheDocument();
    expect(screen.getByText('보습과 톤 개선에 도움')).toBeInTheDocument();
  ...
  it('주의 성분(cautions)을 표시한다', () => {
    render(<AnalysisResultCard result={result} />);
    expect(screen.getByText('향료에 민감한 경우 주의')).toBeInTheDocument();
  });

  it('면책 문구를 항상 표시한다', () => {
    render(<AnalysisResultCard result={result} />);
    expect(screen.getByText('본 결과는 참고용이며 의학적 진단이 아닙니다.')).toBeInTheDocument();
  });
});

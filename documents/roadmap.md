...
- 비고(완료 내역):
  ...
  - **상태 검증**: 품질 계약 테스트 4개 추가(웹/백엔드 CD 트리거·핵심 스텝·시크릿 참조·literal 자격증명 금지·`.env` 플레이스홀더) → `pnpm test:quality` 13개 green. `actionlint` 1.7.12 로 두 워크플로 문법 통과. `pnpm turbo run build --filter=web` 로 `apps/web/dist` 산출 확인.
  - **실제 배포 검증 완료(사용자 확인, 2026-07-22)**: 사용자가 GitHub Secrets/Variables 와 인프라(자체 서버 nginx·S3·CloudFront)를 준비하고 `main` push 로 웹·백엔드 CD 가 실제 배포까지 성공(green)했음을 확인했다. 웹 배포 AWS 인프라: `salka-bucket`, IAM `salka-web-deploy`, 계정 941377156895.

#### R5-CD2 — 모바일 배포(CD) 🔄 진행중
...
- 진행 내역:
  - **코드 파이프라인 완성(자격증명은 사용자 준비 전, 2026-07-22)**: 사용자 결정으로 자격증명·`eas init` 없이 워크플로 코드부터 완성한다(근거: EAS 자격증명·projectId 미확보 상태이며 `eas init` 은 대화형 Expo 로그인이 필요해 비대화형 CI/에이전트 환경에서 실행 불가).
  - **`main` push 트리거 활성화 + 기능 플래그 gate**: `mobile-cd.yml` 에 `push: branches: [main]` 를 추가하되, push 배포는 Variable `MOBILE_CD_ENABLED == 'true'` 일 때만 동작하도록 `decide` job 에서 gate 한다(미설정 시 push 는 `action=none` 으로 안전하게 no-op → 자격증명/projectId 준비 전에도 main push 가 실패하지 않는다). 준비 완료 후 변수만 켜면 활성화되고 워크플로 코드 변경은 불필요하다. 수동 `workflow_dispatch` 는 플래그와 무관하게 항상 동작한다. paths-filter `base: ${{ github.event.before }}` 로 직전 커밋 대비 diff, 네이티브 변경(apps/mobile/package.json·app.json·eas.json, pnpm-lock.yaml) 여부로 build+submit vs OTA 분기.
  - **상태 검증**: 기존 "Mobile CD 는 수동만 허용" 계약 테스트를 R5-CD2 기준(main push + 플래그 gate + build/submit/OTA 분기 + 시크릿/변수 참조)으로 교체 → `pnpm test:quality` 16개 green. `actionlint` 1.7.12 로 `mobile-cd.yml` 문법 통과. `apps/mobile/.env.example` 에 `MOBILE_CD_ENABLED` 를 CI 전용 플래그로 문서화.
  - **남은 작업(사용자 액션 필요 — 완료 기준 충족까지)**: ① 사용자가 `eas init` 실행해 projectId 발급 + `app.json` 에 `owner`/`extra.eas.projectId` 설정, ② `app.json` `ios.bundleIdentifier`/`android.package` 를 실제 값으로 교체(현재 사용자 지시로 플레이스홀더 `com.cosmeticsanalyzer.app` 유지), ③ EAS remote credentials(App Store Connect API Key / Google Play 서비스 계정) 등록 + `eas.json` submit 프로파일의 `ascAppId` 등 실제 값 채우기, ④ GitHub 에 Secret `EXPO_TOKEN`·Variable `EXPO_PUBLIC_API_BASE_URL` 등록, ⑤ 준비 완료 후 Variable `MOBILE_CD_ENABLED=true` 로 켜고 `main` push 로 분기 실동작 검증.
